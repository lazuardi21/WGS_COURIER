	...
	public partial class INVOICERepository
	{
		...
		public void Add(INVOICE invoice)
		{
			using (var conn = new SqlConnection(connString))
			{
				try
				{
					Message = "";
					conn.Open();
					SqlCommand command = new SqlCommand("INSERT INTO Invoice ([id], [invoice_date], [t_o], [ship_to], [payment_type], [sales_name], [courier_name]) VALUES(@id, @invoice_date, @t_o, @ship_to, @payment_type, @sales_name, @courier_name)", conn);
		...
		public void Update(INVOICE invoice)
		{
			using (var conn = new SqlConnection(connString))
			{
				try
				{
					Message = "";
					conn.Open();
					SqlCommand command = new SqlCommand("UPDATE Invoice SET invoice_date = @invoice_date, t_o = @t_o, payment_type = @payment_type, sales_name = @sales_name, courier_name = @courier_name WHERE id = @id AND ship_to = @ship_to", conn);
					command.CommandType = System.Data.CommandType.Text;
					if (invoice.id != null) { command.Parameters.AddWithValue("@id", invoice.id); } else { command.Parameters.AddWithValue("@id", DBNull.Value); } 
					if (invoice.ship_to != null) { command.Parameters.AddWithValue("@ship_to", invoice.ship_to); } else { command.Parameters.AddWithValue("@ship_to", DBNull.Value); } 
					if (invoice.invoice_date != null) { command.Parameters.AddWithValue("@invoice_date", invoice.invoice_date); } else { command.Parameters.AddWithValue("@invoice_date", DBNull.Value); } 
		...
		public void Remove(INVOICE id)
		{
			using (var conn = new SqlConnection(connString))
			{
				try
				{
					Message = "";
					conn.Open();
					SqlCommand command = new SqlCommand("DELETE Invoice WHERE id = @id AND ship_to = @ship_to", conn);
					command.CommandType = System.Data.CommandType.Text;
					if (id.id != null) { command.Parameters.AddWithValue("@id", id.id); } else { command.Parameters.AddWithValue("@id", DBNull.Value); } 
					if (id.ship_to != null) { command.Parameters.AddWithValue("@ship_to", id.ship_to); } else { command.Parameters.AddWithValue("@ship_to", DBNull.Value); } 
					command.ExecuteNonQuery();
				}
				catch (Exception ex)
				{
		...
		public List<INVOICE> GetData()
		{
			...
			using (var conn = new SqlConnection(connString))
			{
				...
				{
					conn.Open();
					SqlCommand command = new SqlCommand("SELECT [id], [invoice_date], [t_o], [ship_to], [payment_type], [sales_name], [courier_name] FROM Invoice", conn);
					...
					while(reader.Read())
					{
						...
						if (reader[1] != DBNull.Value) { item.invoice_date = Convert.ToInt32(reader[1]); }
						...
						if (reader[4] != DBNull.Value) { item.payment_type = Convert.ToInt32(reader[4]); }
						if (reader[5] != DBNull.Value) { item.sales_name = Convert.ToString(reader[5]); }
						if (reader[6] != DBNull.Value) { item.courier_name = Convert.ToInt32(reader[6]); }
						items.Add(item);
					}
				}
				catch (Exception ex)
		...
		public List<INVOICE> GetDataByid(int id)
		{
			...
			using (var conn = new SqlConnection(connString))
			{
				...
				{
					conn.Open();
					SqlCommand command = new SqlCommand("SELECT [id], [invoice_date], [t_o], [ship_to], [payment_type], [sales_name], [courier_name] FROM Invoice WHERE id = @id", conn);
		...
		public List<INVOICE> GetDataByship_to(string ship_to)
		{
			List<INVOICE> items = new List<INVOICE>();
			using (var conn = new SqlConnection(connString))
			{
				...
				{
					conn.Open();
					SqlCommand command = new SqlCommand("SELECT [id], [invoice_date], [t_o], [ship_to], [payment_type], [sales_name], [courier_name] FROM Invoice WHERE ship_to = @ship_to", conn);
					command.Parameters.AddWithValue("@ship_to", ship_to);
					SqlDataReader reader = command.ExecuteReader();
					INVOICE item = new INVOICE();
					while(reader.Read())
					{
		...
		public void RemoveByship_to(string id)
		{
			using (var conn = new SqlConnection(connString))
			{
				try
				{
					Message = "";
					conn.Open();
					SqlCommand command = new SqlCommand("DELETE Invoice WHERE ship_to = @ship_to", conn);
					command.CommandType = System.Data.CommandType.Text;
					if (id != null) { command.Parameters.AddWithValue("@ship_to", id); } else { command.Parameters.AddWithValue("@ship_to", DBNull.Value); } 
					command.ExecuteNonQuery();
				}
				catch (Exception ex)
				{
		...
		public DataTable GetDataTable(List<INVOICE> invoice)
		{
			...
			dt.Columns.Add(c1);
			DataColumn c2 = new DataColumn("invoice_date", typeof(int)); c2.AllowDBNull = true;
			dt.Columns.Add(c2);
			DataColumn c3 = new DataColumn("t_o", typeof(string)); c3.AllowDBNull = true;
			dt.Columns.Add(c3);
			DataColumn c4 = new DataColumn("ship_to", typeof(string)); 
			dt.Columns.Add(c4);
			DataColumn c5 = new DataColumn("payment_type", typeof(int)); c5.AllowDBNull = true;
			dt.Columns.Add(c5);
			DataColumn c6 = new DataColumn("sales_name", typeof(string)); c6.AllowDBNull = true;
			dt.Columns.Add(c6);
			DataColumn c7 = new DataColumn("courier_name", typeof(int)); c7.AllowDBNull = true;
			dt.Columns.Add(c7);
